...
exports.compose = function (manifest /*, [options], [callback] */) {
    ...
    const serverOpts = internals.parseServer(manifest.server || {}, options.relativeTo, options.clone);
    ...
    steps.push((next) => {
        ...
        if (manifest.registrations) {
            const registrations = manifest.registrations.map((reg) => {
                ...
                return {
                    plugin: internals.parsePlugin(reg.plugin, options.relativeTo, options.clone),
                    options: reg.options || {}
                };
            });
...
internals.parseServer = function (server, relativeTo, clone) {
...
internals.parsePlugin = function (plugin, relativeTo, clone) {
